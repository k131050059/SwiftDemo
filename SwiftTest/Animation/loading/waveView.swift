...
class WaveViewConfigure: NSObject {
    //波浪颜色
    var color: UIColor = .white
    //水纹振幅
    var amplitude: CGFloat = 12
    //水纹周期
    var cycle: CGFloat = 0.5/30
    ///波浪Y
    var y: CGFloat = 0
    ///水纹速度
    ...
    ///默认配置
    fileprivate class func defaultConfigure() -> WaveViewConfigure {
        let configure = WaveViewConfigure()
...
class waveView: UIView {
    ...
    private let configure = WaveViewConfigure.defaultConfigure()
