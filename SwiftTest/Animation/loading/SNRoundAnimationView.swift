...
/// - animationMiddle: 中间
/// - animationIn: 内侧
enum SNAnimationPosition: NSInteger {
    case animationOut = 0
    case animationMiddle = 1
    case animationIn = 2
}

class SNRoundAnimationViewConfigure: NSObject {
    ///外圆背景色
    ...
    ///动画位置
    var position: SNAnimationPosition = .animationOut
    
    fileprivate class func defaultConfigure() -> SNRoundAnimationViewConfigure {
        let configure = SNRoundAnimationViewConfigure()
        return configure
    }
}

class SNRoundAnimationView: UIView {
    ...
    private let configure: SNRoundAnimationViewConfigure = SNRoundAnimationViewConfigure.defaultConfigure()
    private var width: CGFloat = 0.0
    private var height: CGFloat = 0.0
    private var isPause: Bool = false
    
    ///更新配置
    func updateWithConfigure(_ configureBlock: ((SNRoundAnimationViewConfigure) -> (Void))?) -> Void {
...
extension SNRoundAnimationView {
    ///开始动画
